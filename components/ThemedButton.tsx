...
const ThemedButton: React.FC<ThemedButtonProps> = ({style, title, children, ...rest}) => {
    ...
    return (
        <TouchableOpacity style={[styles.button, {backgroundColor}, style]} {...rest}>
            <View style={styles.contentContainer}>
                {modifiedChildren}
                {title && <Text style={[styles.buttonText, {color: textColor}, hasChildren && styles.buttonTextWithIcon]}>{title}</Text>}
            </View>
        </TouchableOpacity>
    );
};
